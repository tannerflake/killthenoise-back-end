...
class CalculationService:
    ...
    async def calculate_issue_metrics(
        ...
        async for session in get_db():
            ...
            stmt = select(Issue).where(
                and_(Issue.tenant_id == self.tenant_id, Issue.created_at >= since)
            )
    ...
    async def calculate_source_comparison(
        self, time_range_days: int = 30
    ) -> Dict[str, Any]:
    ...
    async def calculate_severity_distribution(self) -> Dict[str, Any]:
        ...
        async for session in get_db():
            stmt = (
                ...
                .where(
                    and_(Issue.tenant_id == self.tenant_id, Issue.severity.isnot(None))
                )
    ...
    async def calculate_status_distribution(self) -> Dict[str, Any]:
        ...
        async for session in get_db():
            stmt = (
                ...
                .where(
                    and_(Issue.tenant_id == self.tenant_id, Issue.status.isnot(None))
                )
    ...
    async def calculate_sync_health(self, days: int = 7) -> Dict[str, Any]:
        ...
        async for session in get_db():
            ...
            if sync_events:
                durations = [
                    e.duration_seconds for e in sync_events if e.duration_seconds
                ]
            ...
            for event_type, events in by_type.items():
                success_rate = len([e for e in events if e.status == "success"]) / len(
                    events
                )
                type_metrics[event_type] = {
                    "total": len(events),
                    "success_rate": success_rate,
                    "avg_duration": sum(e.duration_seconds or 0 for e in events)
                    / len(events),
                }
    ...
    def _calculate_metrics_from_issues(
        self, issues: List[Issue], time_range_days: int
    ) -> Dict[str, Any]:
    ...
    async def get_top_issues(
        self, limit: int = 10, min_severity: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        async for session in get_db():
            ...
            stmt = stmt.order_by(
                Issue.severity.desc().nullslast(), Issue.created_at.desc()
            ).limit(limit)
    ...
    async def calculate_change_velocity(self, days: int = 30) -> Dict[str, Any]:
        ...
        async for session in get_db():
            ...
            resolved_issues = [
                i for i in issues if i.status in ["resolved", "closed", "done"]
            ]
