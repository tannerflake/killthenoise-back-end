...
class SchedulerService:
    ...
    async def _check_and_sync_integration(self, integration: TenantIntegration) -> None:
        ...
        if time_since_sync >= sync_interval:
            ...
            if integration.id in self.sync_tasks:
                ...
                if not task.done():
                    logger.debug(f"Sync already running for integration {integration.id}")
                    return
                else:
            ...
            # Start new sync task
            logger.info(f"Starting sync for integration {integration.id} ({integration.integration_type})")
            task = asyncio.create_task(
                self._sync_integration(integration),
                name=f"sync_{integration.id}"
            )
    ...
    async def _sync_integration(self, integration: TenantIntegration) -> None:
        ...
        try:
            if integration.integration_type == "hubspot":
                ...
                logger.info(f"HubSpot sync completed for tenant {integration.tenant_id}: {result}")
            
            elif integration.integration_type == "jira":
                # TODO: Implement Jira sync
                logger.info(f"Jira sync not yet implemented for integration {integration.id}")
    ...
    async def get_sync_status(self, tenant_id: Optional[UUID] = None) -> Dict[str, Any]:
        ...
        async for session in get_db():
            ...
            status = {
                "running_tasks": len(self.sync_tasks),
                "integrations": []
            }
    ...
    async def calculate_sync_metrics(self, tenant_id: UUID, days: int = 7) -> Dict[str, Any]:
